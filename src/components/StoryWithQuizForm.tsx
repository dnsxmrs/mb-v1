...
export default function StoryWithQuizForm({ story, onSuccess, onCancel }: StoryWithQuizFormProps) {
    ...
    const validateForm = () => {
        ...
        if (quizItems.length === 0) {
            ...
            for (let i = 0; i < quizItems.length; i++) {
                ...
                const validChoices = item.choices.filter(choice => choice.trim() !== '')
                if (validChoices.length < 2) {
                    newErrors.quiz = `Question ${i + 1} must have at least 2 answer choices`
                    break
                }
                
                if (!item.correctAnswer || !item.choices.includes(item.correctAnswer)) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const cleanedQuizItems = quizItems.map((item, index) => ({
                ...item,
                quizNumber: index + 1,
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                                            errors.fileLink ? 'border-red-500' : 'border-gray-300'
                                        }`}
